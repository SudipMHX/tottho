...
export default async function AdminShowcasePage() {
  ...
  const [profilesRaw, total]: [IProfileDoc[], number] = await Promise.all([
    Profile.find({})
  ...
  const serialized = profilesRaw.map((p: IProfileDoc) => ({
    _id: String(p._id),
    ...
    <div className="p-8 animate-fade-in">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Toggle profiles to feature them on the public <Link href="/showcase" className="text-[#FF5240] underline hover:text-[#FF5240]/80 transition-colors">Showcase page</Link>.
        </p>
      </div>
